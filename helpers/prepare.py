...
def get_tomorrow_date(date_format='%Y-%m-%d'):
    """
    Метод получения завтрашней даты
    :param date_format: формат даты (по умолчанию YYYY-MM-DD)
    :return: завтрашняя дата в указанном формате
    """
    return (datetime.now() + timedelta(days=1)).strftime(date_format)


def get_day_after_tomorrow_date(date_format='%Y-%m-%d'):
    """
    Метод получения послезавтрашней даты
    :param date_format: формат даты (по умолчанию YYYY-MM-DD)
    :return: послезавтрашняя дата в указанном формате
    """
    return (datetime.now() + timedelta(days=2)).strftime(date_format)
