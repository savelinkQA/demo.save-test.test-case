...
class ClassCheck:
    ...
    def check_value_equals_today_date(self, locator: Locator, name='', date_format='%d-%m-%Y', **kwargs):
        """
        Метод проверки атрибута value элемента на совпадение с сегодняшней датой
        :param locator: локатор элемента
        :param name: имя элемента. Только для allure
        :param date_format: формат даты (по умолчанию DD-MM-YYYY)
        :param kwargs: опции метода to_have_attribute()
        """
        today = datetime.now().strftime(date_format)
        with allure.step(f'Проверка атрибута value={today} у элемента {name}'):
            expect(locator).to_have_attribute('value', today, **kwargs)
